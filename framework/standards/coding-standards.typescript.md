...
## 0. 项目级编码规范（扫描驱动 — 最高优先级）

本章节由 Scanner Agent 在 Phase 0 初始化时自动生成，是从项目实际代码中检测到的 **权威编码约定**。**所有智能体编码时必须优先遵守本章节**。后续通用语言规范仅在本章节未覆盖时适用。当两者冲突时，**以本章节为准**。

### 0.1 检测机制与优先级
Scanner Agent 在 Phase F 对全量代码采样分析（每个模块最多 20 个文件），覆盖率 >70% 认定为 **项目主导约定**。

标记含义：
- ✓ **confirmed** — 与通用标准一致，通用规则保持
- ⚠ **overridden** — 与通用标准不同，**项目约定覆盖通用规则**
- ◈ **generic** — 通用规则，不可由扫描检测（如安全规范）
- ⬢ **remediation** — 覆盖率 <70%，存在不一致需整改
- ⬢ **project-specific** — 项目特有约定，通用标准未定义，**作为强制规则新增**
...
**生成规则：**
- 新常量 MUST 使用检测到的命名规范（`UPPER_SNAKE_CASE` / `camelCase` / 其他）
- 新枚举定义 MUST 使用检测到的类型偏好（`enum` / `as const` / 其他）
...
2. **本章节标记为 ⬢ project-specific** → 作为项目强制规则执行，通用标准无对应条目
3. **本章节标记为 ✓ confirmed** → 通用规则适用，本章节仅作确认记录
4. **本章节标记为 ◈ generic** → 通用规则适用，无法自动检测
