...
### 3. Load Environment
Read `docs/architecture.md` and `docs/module-map.md` (including the keyword-to-module index), load coding standards, scan `plugins/` for hooks, verify framework rules accessible. Identify the current requirement directory. For code-level work, consult the keyword index in module-map.md to locate relevant modules and their capabilities before writing new code.
